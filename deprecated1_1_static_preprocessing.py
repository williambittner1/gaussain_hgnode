...
def compute_arap_loss(gaussians, ref_positions, neighbor_radius=0.1):
    """
    Computes an ARAP loss which penalizes deviations in the pairwise distances
    (for nearby Gaussians) relative to the initial configuration (ref_positions).
    """
    current_positions = gaussians._xyz  # Assumes shape (N, 3)
    # Compute pairwise differences
    diff_current = current_positions.unsqueeze(1) - current_positions.unsqueeze(0)  # (N, N, 3)
    diff_ref = ref_positions.unsqueeze(1) - ref_positions.unsqueeze(0)              # (N, N, 3)
    # Compute reference pairwise distances
    dist_ref = torch.norm(diff_ref, dim=-1)  # (N, N)
    # Create a neighbor mask (ignore self-comparisons and only consider neighbors within threshold)
    neighbor_mask = (dist_ref > 1e-5) & (dist_ref < neighbor_radius)
    # Compute current pairwise distances
    current_norm = torch.norm(diff_current, dim=-1)
    # The loss is the squared difference between the current and reference distances for neighbors
    loss = torch.sum(((current_norm - dist_ref) ** 2)[neighbor_mask])
    num_pairs = neighbor_mask.sum()
    if num_pairs > 0:
        loss = loss / num_pairs
    return loss
...
def train_GM1_arap(scene, config, iterations=3000):
    """
    Refines the GM0 model (t=0) to obtain GM1 (t=1) by training with photometric loss
    and an additional ARAP loss term. Densification and pruning are disabled to preserve point correspondences.
    """
    bg_color = [0, 0, 0]
    background = torch.tensor(bg_color, dtype=torch.float32, device="cuda")
    
    # Store the initial positions from GM0 as a reference for ARAP.
    ref_positions = scene.gaussians._xyz.clone().detach()

    scene.gaussians.training_setup_arap(config.optimization)
    
    viewpoint_stack = scene.getTrainCameraObjects()
    progress_bar = tqdm(range(iterations), desc="Refining GM1 with ARAP")
    
    for iteration in range(iterations):
        # Select a random camera viewpoint (same as before)
        viewpoint_cam = select_random_camera(scene)
        gt_image = viewpoint_cam.original_image.permute(2, 0, 1)
        render_pkg = render(viewpoint_cam, scene.gaussians, config.pipeline, background)
        rendered_image = render_pkg["render"]
        
        # Photometric loss (MSE between rendered image and ground truth)
        loss_photo = F.mse_loss(rendered_image, gt_image)
        
        # ARAP loss: penalize changes in local pairwise distances
        arap_loss = compute_arap_loss(scene.gaussians, ref_positions)
        
        total_loss = loss_photo + config.optimization.arap_weight * arap_loss
        ...
        progress_bar.set_postfix({
            "Photo Loss": f"{loss_photo.item():.7f}",
            "ARAP Loss": f"{arap_loss.item():.7f}",
            "Total Loss": f"{total_loss.item():.7f}",
            "Num Gaussians": f"{len(scene.gaussians._xyz)}"
        })
        progress_bar.update(1)
    progress_bar.close()
...
def train(config):
    ...
    for seq_idx in range(len(dataset)):
        ...
        gm1_iterations = config.optimization.arap_iterations
        train_GM1_arap(scene, config, iterations=gm1_iterations)
