...
def train():
    ...
    for epoch in epoch_bar:
        ...
        for scene_idx in range(len(dataset)):
            ...
            ##########################################
            # Load Gaussian Models for t=0 and t=1 (GM0, GM1)
            ##########################################
            ...
            ##########################################
            # Dense Gaussians at t=0
            ##########################################
            # Each Gaussian has [position, velocity, quat, omega, abc, wedge, dt_wedge,color, semantic features]

            # position = GM1.pos
            # velocity = GM1.pos - GM0.pos
            # quat = GM1.quat
            # omega (angular velocity) = quat_delta_to_angular_velocity(q1 * inverse(q0))
            # abc (3 nearest neighbors of each gaussian inGM0) via k-NN
            # wedge (volume spanned by 3 nearest neighbor vectors abc)
            # dt_wedge (relative orientation change) via finite difference GM1.wedge - GM0.wedge
            # color (constant)
            # semantic features (constant)
